...
class NestedTensor(torch.Tensor):
    ...
    def __init__(self, values, offsets, *, lengths=None, **kwargs) -> None:
        ...
        for tensor, dim in [
            (self, self._ragged_idx),
            (self._values, self._ragged_idx - 1),
        ]:
            if hasattr(tensor, "_dynamo_propagated_dynamic_indices"):
                tensor._dynamo_propagated_dynamic_indices.add(dim)  # type: ignore[union-attr]
            else:
                tensor._dynamo_propagated_dynamic_indices = {dim}  # type: ignore[attr-defined]
